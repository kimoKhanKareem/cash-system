...
export const CartProvider = ({ children }) => {
    ...
    const TotalPriceQty = () =>{
        let newTotal = keyitem.reduce((prev, curr) =>prev + curr.result.join('') * curr.pric.join('') , 0);
    ...
    // const [ lo, setLo] = useState([])
    // const pol = (item) =>{
    //     let CartTotall = item.reduce((prev, curr) =>!curr.result.length && !curr.pric.length && !curr.dis.length ? prev + curr.qty * curr.price: curr.result.join('') * curr.price, 0);
    //     setLo(CartTotall)

        // setLo(!item.result.length && !item.pric.length && !item.dis.length ?
